...
function calendar(obj_target, approot) {
	// assigning methods
...
function cal_popup2(str_datetime) {
	...
	var obj_calwindow = window.open(
		this.approot + '/script/calendar.html?'
			+ 'approot=' + this.approot
			+ '&datetime=' + this.dt_current.valueOf()
			+ '&id=' + this.id,
		'Calendar', 'width=270,height=' + (this.time_comp ? 215 : 190) +
...
function cal_prs_date2(str_date) {
	...
	if (!RE_NUM.exec(arr_date[1]))
		return cal_error("Invalid day of month value: '" + arr_date[1] + "'.\nAllowed values are unsigned integers.");
	if (!arr_date[0])
